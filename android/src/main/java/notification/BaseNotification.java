...
public abstract class BaseNotification implements INotification
{
  ...
  public BaseNotification(String pId, String pCreator, long pDate, String pTarget, boolean pVisibleForTarget)
  {
    id = pId;
    creator = pCreator;
    startDate = new ValueFromActionTemplate<>(new NotificationStartDate(new Date(pDate)));
  ...
  {
    return startDate.getValue().getDate().getTime();
  }
  ...
  private void _setupButtonActions(final Activity pContext)
  {
    startDate.setButtonAction(new IButtonAction<NotificationStartDate>()
    {
      ...
      {
        Date currDate = startDate.getValue() != null ? startDate.getValue().getDate() : new Date(System.currentTimeMillis());
